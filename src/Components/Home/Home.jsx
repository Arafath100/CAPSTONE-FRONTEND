...
const Home = () => {
  ...
  useEffect(() => {
    if (flag) {
      verifyTokenAxios()
        .then((res) => {
          if (res.status === 200) {
            // Displaying a welcome message using a toast notification
            setFlag(false);
          }
        })
        .catch((err) => {
  ...
  return (
    ...
    >
      <img
        src="https://img.freepik.com/premium-vector/flat-design-icon-postman_362714-180.jpg?w=2000"
        alt="Home"
        ...
        <div>
          <Card style={{ width: "" }} className="text-start">
            <Card.Body>
              ...
              <hr />
              <Card.Subtitle className="mb-2 w-100 text-muted d-flex">
                Email Sent <span className="ms-auto me-5 ">{count.count}</span>
              </Card.Subtitle>
              <br />
