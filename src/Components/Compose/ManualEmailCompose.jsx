...
export function ManualEmailCompose({ reUse }) {
  ...
  // Formik initialization and configuration
  const {
    values,
    handleChange,
    setFieldTouched,
    ...
    initialValues: init,
    onSubmit: (values) => {
      // Code to handle form submission and send email
      setEnterFlag(true);
      const { emails } = values;
  ...
  // Function to process and clean email data for duplicate removal
  function dataModal(val) {
    ...
    let obj = {
      duplicates: dataMod.length - arr.length,
  ...
  // JSX for ManualEmailCompose component
  return (
    <div className="d-flex justify-content-center align-items-center m-4 composestyle">
          ...
          <Form.Group controlId="formField " className="mb-3">
            ...
            {errors.emails && touched.emails ? (
              ...
              <Form.Text id="mailHelpBlock" muted className="text-start d-flex">
                Enter email address above seperated by comma &emsp; <b> Eg: </b>
