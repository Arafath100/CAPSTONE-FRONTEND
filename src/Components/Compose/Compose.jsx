...
const Compose = () => {
  ...
  // Accessing context and router functionalities
  const contextData = useContext(Context);
  const navigate = useNavigate();
  const da = useLocation();

  // Radio button options for manual and bulk email send
  const radios = [
  ...
  // File change handler for bulk email send
  const handleFile = (e) => {
  ...
  // useEffect to reset form and clear recipient info when radio value changes
  useEffect(() => {
    resetForm();
    setRecepaintInfo([]);
  }, [radioValue]);
